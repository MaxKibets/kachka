...
> Активний екран: архітектура зон (§5), логування сета + custom numpad (§7), меню дій з сетом (§8). Суперсети — окремо у `supersets.md`. Частина UI/UX-специфікації Kachka v1 — повна карта і §-індекс: [spec map](README.md).
