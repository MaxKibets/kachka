...
**Версія**: v0.6 · add Exercise picker / Database section
...
- **Browse mode** — root screen у Profile stack. Тап на custom-рядку → detail. Тап на system-рядку — нічого. Header: `Exercise database`, без back
...
│  (or Exercise database) │  Browse mode header
├─────────────────────────┤
│  🔍 Search exercises    │
├─────────────────────────┤
...
Hard delete (видалити з History назавжди) у v1 не передбачено. Якщо колись треба — окрема дія "Wipe from history" у Settings → Delete data.
...
## 14. Свідомо відкладено в v2 / пізніше
