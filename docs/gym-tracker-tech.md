...
### 6.3 Deep linking — у v2

Спочатку закладалось з першого дня для шарингу програм. Оскільки програми відкладено у v2, deep linking теж переноситься у v2:
...
v1 не використовує deep links. Якщо колись буде sharing single-workout / лога — заведемо окремий тип payload-у поверх тієї ж інфраструктури.
...
- [x] v1 scope = ad-hoc workouts (програми / import / deep linking → v2)
- [x] Exercise database з seed-списком 7 popular exercises (Squat, Bench Press, Deadlift, Barbell Row, Overhead Press, Pull-up, Bicep Curl) для Quick-add chips у Builder
...
- Exercise database: де брати seed-список для повного каталогу (open-source бази типу wger, або власний). 7 вправ для Quick-add chips зашиваємо у застосунок, але повний каталог — окреме питання
- Чи потрібна синхронізація з Apple Health / Google Fit
- Drag-and-drop бібліотека для reorder в Builder і Active workout (`react-native-draggable-flatlist` як кандидат)
